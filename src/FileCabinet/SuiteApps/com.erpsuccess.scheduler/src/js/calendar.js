...
setTimeout(() => {
  ...
  Toastify({
    ...
    // backgroundColor: "#ff0000"
    style: {
      background: 'linear-gradient(to right, #552956, #8E1F06)',
    }
  }).showToast();

  Toastify({
    text: 'Drag Available Jobs to the Calendar',
    duration: 99999,
    // destination: '/contact',
    // newWindow: true,
    close: true,
...
// *************** LAYOUT ***************
const calendarBaseWidth = calendar.getBoundingClientRect().width;
...
toggleRight.addEventListener('click', el => {
  if (collapseRight.style.display === 'none' || collapseRight.style.display === '') {
    collapseRight.style.display = 'block';
    rightSidebar.style.width = '18%'; // Adjust width as needed
    toggleRight.classList.remove('fa-square-caret-left');
    toggleRight.classList.add('fa-square-caret-right');
  } else {
    collapseRight.style.display = 'none';
    rightSidebar.style.width = '0'; // Adjust width as needed
    toggleRight.classList.remove('fa-square-caret-right');
    toggleRight.classList.add('fa-square-caret-left');
  }
  zoomElementByPx();
});

function zoomElementByPx() {
  console.log('Base Width', calendarBaseWidth);
  const _10 = calendarBaseWidth * .1;
  const _20 = calendarBaseWidth * .2;
  let zoomPx = 0;

  if (!parseFloat(rightSidebar.style.width) && !parseFloat(leftSidebar.style.width)) {
    zoomPx = _20;
    console.log('Condition A', zoomPx);
  } else if (!parseFloat(rightSidebar.style.width) || !parseFloat(leftSidebar.style.width)) {
    zoomPx = _10;
    console.log('Condition B', zoomPx);
  } else {
    zoomPx = 0;
    console.log('Condition C', zoomPx);
  }

  const newWidth = currentWidth + zoomPx;
  calendar.style.width = newWidth + 'px';

  globalVar.calendar.updateSize();
}

function setDefaultSideBarsWidth() {
  collapseLeft.style.display = 'block';
  leftSidebar.style.width = '18%';
  collapseRight.style.display = 'block';
  rightSidebar.style.width = '18%';
}

// *************** CARD ACTIONS ***************
function showMenuOptions(ev) {
  console.log('EV', ev)
...
window.onclick = (e) => {
  if (!e.target.classList || !e.target.classList.contains('expand')) {
...
globalVar.eventAction = '';
globalVar.suiteletUrl = '';
globalVar.workOrders = [];
globalVar.resources = [];
globalVar.resourceGroups = [];
globalVar.temp_woLinesDataTable = null;
globalVar.temp_woId = null;
globalVar.temp_eventId = null;
// globalVar.addEvent = {};
