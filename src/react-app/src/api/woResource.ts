...
export const fetchWOResources = async (eventId: string): Promise<WOResource[]> => {
  ...
  try {
    ...
    while (hasMoreData) {
      ...
      const url = `${suiteletUrl}&mode=getWorkOrderResources&eventId=${eventId}&start=${start}&end=${end}`;
