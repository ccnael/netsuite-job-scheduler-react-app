...
export const WOAddressTable: React.FC<WOAddressTableProps> = ({ woId, onSelectionChange, onUpdate, selectedEvent }) => {
  ...
  const columns: ColumnDef<TableWOAddress>[] = useMemo(() => [
  {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-slate-900 text-[12px] font-sans tracking-tight">{row.getValue("customer")}</div>
      ),
    },
    {
      accessorKey: "address",
      ...
      cell: ({ row }) => (
        <div className="text-slate-900 text-[12px] font-sans tracking-tight">{row.getValue("address")}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-[12px] font-sans text-slate-900 tracking-tight text-center">
